...
![SSH 키 기반 인증 과정.png](SSH 키 기반 인증 과정.png)

1. 클라이언트는 비대칭키 쌍(개인 키와 공개 키)을 생성한 후, 생성된 공개 키를 서버의 '$HOME/.ssh/authorized_keys' 파일에 등록합니다.
2. SSH 접속 시 '사용자@IP' 로 서버에 연결을 요청합니다.
3. 서버는 클라이언트에게 인증 가능한 방법들을 응답합니다.
4. 클라이언트는 publickey 방식을 선택하고, 자신이 인증에 사용할 공개 키의 식별자(fingerprint)와 로그인하려는 사용자 이름을 서버에 제시합니다.
5. 서버는 해당 정보들로 'authorized_keys' 파일에서 공개 키를 찾습니다.
6. 서버는 찾은 공개 키에 대응하는 개인 키를 가진 클라이언트만이 응답할 수 있는 임의의 챌린지 데이터(종종 클라이언트의 공개 키로 암호화된 형태)를 생성하여 클라이언트에게 보냅니다.
7. 클라이언트는 서버로부터 받은 챌린지 데이터를 자신의 개인 키로 복호화하거나 서명한 후, 서버로 다시 전송합니다.
8. 서버는 클라이언트가 보낸 응답(복호화된 데이터 또는 서명)을 자신의 공개 키로 검증합니다. 만약 클라이언트의 응답이 성공적으로 검증되면, 서버는 클라이언트와 안전하게 세션 키(대칭 키)를 협상하고 공유합니다.
9. 이후 해당 SSH 세션 동안의 모든 데이터 암호화 및 복호화는 이 세션 키(대칭 키)로 진행됩니다.
